-- =============================================================================
-- MÓDULO TORNEOS LDDBI — SQL COMPLETO (Fases A + B + C + D)
-- Ejecutar en Supabase → SQL Editor → Run
-- Requisitos previos: tablas public.leagues y public.categories deben existir
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.tournament_matches (
  ...
  home_ot smallint,
  away_ot smallint,
  playoff_label varchar(100),
  advances_to_match_id uuid REFERENCES public.tournament_matches(id) ON DELETE SET NULL,
  advances_as varchar(10),
  loser_advances_to_match_id uuid REFERENCES public.tournament_matches(id) ON DELETE SET NULL,
  loser_advances_as varchar(10),
  notes text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS tournament_matches_phase_idx ON public.tournament_matches (tournament_id, phase);
...
-- Columnas extra si ya existían tablas de una migración parcial (0013 sin D/C)
...
ALTER TABLE public.tournament_matches
  ...
  ADD COLUMN IF NOT EXISTS advances_to_match_id uuid REFERENCES public.tournament_matches(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS advances_as varchar(10),
  ADD COLUMN IF NOT EXISTS loser_advances_to_match_id uuid REFERENCES public.tournament_matches(id) ON DELETE SET NULL,
