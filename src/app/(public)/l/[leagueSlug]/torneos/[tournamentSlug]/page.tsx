...
export default async function LeaguePublicTournamentPage({
  params,
}: {
  ...
  const homeHref = leaguePortalHome(league.slug);

  return (
    <div className="flex min-h-screen flex-col bg-[#F5F5F5]">
      <PortalSiteHeaderBar
        panelHref={panelHref ?? "/login/"}
        leagueSlug={league.slug}
        leagueName={league.name}
        leagueLogoUrl={league.logoUrl}
      />
      <main className={`mx-auto max-w-5xl flex-1 px-4 py-6 sm:py-8 ${PORTAL_SHELL_CLASS}`}>
        <Link href={`${homeHref}#campeonatos`} className="text-xs font-medium text-[#1B3A6B] hover:underline">
          ← Campeonatos · {league.name}
        </Link>
        <div className="mt-4">
