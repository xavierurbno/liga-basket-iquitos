...
export async function resolveClubBySlugForPortal(
  ...
  if (leagueId) {
    return clubRepository.findBySlugAndLeague(slug, leagueId);
  }

  const leagueSlug = opts?.leagueSlug?.trim() || null;
  if (leagueSlug) {
    const league = await leagueRepository.findBySlug(leagueSlug);
    if (league) {
      return clubRepository.findBySlugAndLeague(slug, league.id);
    }
    return null;
  }

  return clubRepository.findBySlugUnambiguous(slug);
}
